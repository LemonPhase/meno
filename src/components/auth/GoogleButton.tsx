...
/**
 * One button, because Google sign-in has no separate sign-up: a first
 * sign-in creates the Graph, every later one opens it.
 */
  ...
  return (
    <div>
      <button className="gbtn" onClick={go} disabled={busy}>
        {/* Scripted mode is not Google, so it does not wear Google's mark. */}
        {!scriptedAuth && <GoogleMark />}
        <span>
          {busy
            ? "Signing in…"
            : scriptedAuth
              ? "Continue as the demo reader"
